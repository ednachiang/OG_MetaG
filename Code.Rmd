...
### Test removing CAZymes with count < 3
# 3 samples per season, so I went with the somewhat arbitrary number of 3
cut.phy <- prune_taxa(taxa_sums(dbcan.phy) > 2, dbcan.phy)
cut.otu <- data.frame(otu_table(cut.phy))


dbcan.dds <- DESeqDataSetFromMatrix(countData = cut.otu, colData = meta, design = ~ Season)
dbcan.dds <- estimateSizeFactors(dbcan.dds)
dbcan.norm <- counts(dbcan.dds, normalized=T)
#write.csv(dbcan.norm, file="dbcan/cut3.counts.csv")
dbcan.fam.norm <- dbcan.norm
...
### Test cut.phy (removed CAZymes w/ < 3 counts)
### Convert phyloseq object to deseq2 object
dbcan.dds <- phyloseq_to_deseq2(cut.phy, ~Season)
  # Design variable must be unordered otherwise you get an error
dbcan.dds$Season <- factor(dbcan.dds$Season, levels=c("Summer", "Winter", "Spring"))
dbcan.dds <- dbcan.dds[rowSums(counts(dbcan.dds)) > 1,]
...
mucin.sum <- mucin.df %>%
  ...
  summarize(Mean = mean(Abundance),
            SD= sd(Abundance),
            SE = se(Abundance))
mucin.sum$FullName = ordered(mucin.sum$FullName, c("GH2_3.2.1.23", "GH2_3.2.1.25", "GH3_3.2.1.52", "GH4_3.2.1.22", "GH4_3.2.1.24", "GH16_3.2.1.23", "GH16_3.2.1.103", "GH20_3.2.1.52", "GH27_3.2.1.22", "GH29_3.2.1.51", "GH31_3.2.1.20", "GH35_3.2.1.23", "GH36_3.2.1.22", "GH38_3.2.1.24", "GH42_3.2.1.23", "GH67_3.2.1.139", "GH84_3.2.1.169", "GH95_3.2.1.51", "GH101_3.2.1.97", "GH110_3.2.1.22", "GH112_2.4.1.211", "GH130_2.4.1.281", "GH130_2.4.1.320", "3.1.1.2", "3.1.6.3", "3.1.6.4", "3.1.6.8", "3.1.6.14", "4.1.3.3"))
...
plot_heatmap(mucin.sig, sample.label = "Season", sample.order = c("3715", "3717", "3744", "3723", "3733", "3772", "3734", "3773", "3775"), taxa.order = c("GH110_3.2.1.22", "GH84_3.2.1.169", "GH42_3.2.1.23", "GH36_3.2.1.22", "GH112_2.4.1.211")) +
  geom_tile(colour="gray40") +
  geom_text(aes(label=sprintf("%0.2f", round(Abundance, digits=4)))) +
  scale_x_discrete(position = "top", labels=c("Summer", "Summer", "Summer", "Winter", "Winter", "Winter", "Spring", "Spring", "Spring")) +
  xlab("Season\n\n") +
  ylab("Mucin-Degrading Enzyme Gene\n\n") +
  scale_fill_gradientn(name=expression("Normalized Counts"), limits=c(0, 100),
