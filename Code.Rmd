...
# DESeq DataStructure for analyses; rlog-transformed
...
# I'll make a separate plot for just GT4, GT41, GH43, GH3
dbcan.sig <- subset_taxa(dbcan.fam.norm.phy, Family == "GH101" | Family == "PL1" | Family == "GH115" |
                           Family == "PL9" | Family == "CE12" | Family == "GH10" | Family == "GH32" |
                           Family == "GH5" | Family == "GH51" | Family == "GH36" | Family == "GH25" |
                           Family == "GH78" | Family == "GH42" | Family == "GH129" | Family == "GH106" |
                           Family == "GH39" | Family == "GH59" | Family == "GH14" | Family == "PL22" |
                           Family == "GH41" | Family == "GH20" | Family == "GH92" | Family == "GT9" |
...
sig <- c("GT101", "GH10", "GH43", "GH59", "GH115", "CE12", "PL1", "GH3", "GH5", "GH32", "GH39", "GH42", "GH78", "GH106", "GH20", "GH84", "GT3", "GT4", "GT9", "GT41", "CE11")
dbcan.rl.use <- assay(dbcan.rl)


for (i in 1:length(sig)){
  
  if (i == 1){
    num <- which(rownames(dbcan.rl.use) == sig[i])
    dbcan.sig <- t(data.frame(dbcan.rl.use[num,]))
    dbcan.sig <- data.frame(dbcan.sig)
    rownames(dbcan.sig)[i] = sig[i]
  }
  
  num <- which(rownames(dbcan.rl.use) == sig[i])
  dbcan.sig[i,] <- dbcan.rl.use[num,]
  rownames(dbcan.sig)[i] = sig[i]
}
colnames(dbcan.sig) <- c("3715", "3717", "3723", "3733", "3734", "3744", "3772", "3773", "3775")
dbcan.sig.phy <- merge_phyloseq(otu_table(dbcan.sig, taxa_are_rows = T), tax, sample_data(meta.ord))
...
"CE11", "GT41", "GT9", "GT4", "GT3", "GH84", "GH20", "GH106", "GH78", "GH42", "GH39", "GH32", "GH5", "GH3", "PL1", "CE12", "GH115", "GH59", "GH43", "GH10", "GT101"

#tiff("dbcan.heatmap.tiff", width=9, height=10, units="in", res=300)
plot_heatmap(dbcan.sig.phy, sample.label = "Season", sample.order = c("3715", "3717", "3744", "3723", "3733", "3772", "3734", "3773", "3775"), taxa.order = c("CE11", "GT41", "GT9", "GT4", "GT3", "GH84", "GH20", "GH106", "GH78", "GH42", "GH39", "GH32", "GH5", "GH3", "PL1", "CE12", "GH115", "GH59", "GH43", "GH10", "GT101")) +
  geom_tile(colour="gray40") +
  scale_x_discrete(position = "top", labels=c("Summer", "Summer", "Summer", "Winter", "Winter", "Winter", "Spring", "Spring", "Spring")) +
  xlab("Season\n\n") +
  ylab("CAZyme Family\n\n") +
  scale_fill_gradientn(name=expression("Normalized Counts"), limits=c(0,10.2),
