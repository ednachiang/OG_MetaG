...
# Summer vs. Winter
dbcan.sw.bray <- phyloseq::distance(dbcan.sw, method="bray")
# Season
adonis(dbcan.sw.bray~Season, data=data.frame(sample_data(dbcan.sw)), permutations = 99999)
  # p-value = 0.1
beta.tax = betadisper(d=dbcan.sw.bray, group=data.frame(sample_data(dbcan.sw))$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.6013889

# Summer vs. Spring
dbcan.sp.bray <- phyloseq::distance(dbcan.sp, method="bray")
# Season
adonis(dbcan.sp.bray~Season, data=data.frame(sample_data(dbcan.sp)), permutations = 99999)
  # p-value = 0.1
beta.tax = betadisper(d=dbcan.sp.bray, group=data.frame(sample_data(dbcan.sp))$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.9013889

# Winter vs. Spring
dbcan.wp.bray <- phyloseq::distance(dbcan.wp, method="bray")
# Season
adonis(dbcan.wp.bray~Season, data=data.frame(sample_data(dbcan.wp)), permutations = 99999)
  # p-value = 0.1
beta.tax = betadisper(d=dbcan.wp.bray, group=data.frame(sample_data(dbcan.wp))$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.1013889
