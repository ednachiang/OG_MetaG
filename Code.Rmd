...
### dbCAN - Test assembler effect on class-level results
```{r}
# Load dbcan class-level rel abund
dbcan <- read.csv('dbcan/idba.spades.comparison.csv', row.names=1)
colnames(dbcan) <- c("3715_idba", "3715_spades", "3717_idba", "3717_spades", "3723_idba", "3723_spades", "3733_idba", "3733_spades", "3734_idba", "3734_spades", "3744_idba", "3744_spades", "3772_idba", "3772_spades", "3773_idba", "3773_spades", "3775_idba", "3775_spades")

# Load metadata
dbcan.meta <- read.csv('dbcan/dbcan.metadata.csv', row.names=1)
dbcan.meta$Season <- ordered(dbcan.meta$Season, levels=c("Summer", "Winter", "Spring"))
dbcan.meta$Squirrel <- ordered(dbcan.meta$Squirrel, levels=c("3715", "3717", "3723", "3733", "3734",
                                                             "3744", "3772", "3773", "3775"))

# Create physeq object
dbcan.physeq <- merge_phyloseq(otu_table(dbcan, taxa_are_rows=T), sample_data(dbcan.meta))

# Melt data together
dbcan.melt <- psmelt(dbcan.physeq)
dbcan.melt$OTU <- ordered(dbcan.melt$OTU, levels=c("AA", "CBM", "CE", "GH", "GT", "PL", "SLH"))
dbcan.melt$Sample <- ordered(dbcan.melt$Sample, levels=c("3715_idba", "3715_spades", "3717_idba",
                                                         "3717_spades", "3723_idba", "3723_spades",
                                                         "3733_idba", "3733_spades", "3734_idba",
                                                         "3734_spades", "3744_idba", "3744_spades",
                                                         "3772_idba", "3772_spades", "3773_idba",
                                                         "3773_spades", "3775_idba", "3775_spades"))

# Examine repeated measures
summary(lmer(Abundance ~ Assembler + (1|Squirrel), data = dbcan.melt))
  # Little variance in data is explained by individual squirrel random effects (5.207e-19)
  # That means we don't have to account for individual squirrels when we test whether abundance is sig influenced by assembler

# Test Assembler
summary(aov(Abundance ~ Assembler, data = dbcan.melt))
  # p-value = 1
  # Abundance isn't sig diff btwn assemblers
  # Aka it doesn't matter if you use IDBA or metaSPADes -- the class-level dbcan results will be the same
```

### dbCAN - Class-level bar plot
```{r}
# Load dbcan class-level rel abund
dbcan <- read.csv('dbcan/spades.dbcan.csv', row.names=1)
colnames(dbcan) <- c("3715", "3717", "3723", "3733", "3734", "3744", "3772", "3773", "3775")

# Load metadata
dbcan.meta <- read.csv('dbcan/dbcan.spades.metadata.csv', row.names=1)
dbcan.meta$Season <- ordered(dbcan.meta$Season, levels=c("Summer", "Winter", "Spring"))
...
# Plot barplot
ggplot(dbcan.sum, aes(x=Season, y=Mean, fill=Season)) +
  geom_bar(stat="identity", color="black") +
  geom_errorbar(aes(ymax=Mean+SE, ymin=Mean-SD), width=0.25) +
...
```{r}
### AA
aa <- dbcan.melt[which(dbcan.melt$OTU == "AA"),]

# Test normality
hist(aa$Abundance)
qqnorm(aa$Abundance)
qqline(aa$Abundance)
shapiro.test(aa$Abundance)
  # p-value = 0.8563

# Test AA w/ ANOVA
summary(aov(Abundance ~ Season, data = aa))
  # p-value = 0.208
  # Normal


### CBM
cbm <- dbcan.melt[which(dbcan.melt$OTU == "CBM"),]

# Test normality
hist(cbm$Abundance)
qqnorm(cbm$Abundance)
qqline(cbm$Abundance)
shapiro.test(cbm$Abundance)
  # p-value = 0.07196
  # Normal

# Test CBM w/ ANOVA
summary(aov(Abundance ~ Season, data = cbm))
  # p-value = 0.484


### CE
ce <- dbcan.melt[which(dbcan.melt$OTU == "CE"),]
...
### GH
gh <- dbcan.melt[which(dbcan.melt$OTU == "GH"),]
...
TukeyHSD(aov(Abundance ~ Season, data = gh))
  # Winter-Summer = 0.0169264*
  # Spring-Summer = 0.3384504
  # Spring-Winter = 0.0035475*


### GT
gt <- dbcan.melt[which(dbcan.melt$OTU == "GT"),]
...
TukeyHSD(aov(Abundance ~ Season, data = gt))
  # Winter-Summer = 0.2800447
  # Spring-Summer = 0.0737956
  # Spring-Winter = 0.0102155*


### PL
pl <- dbcan.melt[which(dbcan.melt$OTU == "PL"),]
...
### SLH
slh <- dbcan.melt[which(dbcan.melt$OTU == "SLH"),]

# Test normality
hist(slh$Abundance)
qqnorm(slh$Abundance)
qqline(slh$Abundance)
shapiro.test(slh$Abundance)
  # p-value = 0.358
  # Normal

# Test SLH w/ ANOVA
summary(aov(Abundance ~ Season, data = slh))
  # p-value = 0.16


### Adjust Pvals
p.adjust(c(0.208, 0.484, 0.831, 0.00376, 0.012, 0.0829, 0.16), method="BH")
  # AA = 0.2912
  # CBM = 0.5646667
  # CE = 0.831
  # GH = 0.02632 *
...
### dbCAN - Create phyloseq object to prep for family-level & b-div
```{r}
# Load data
...
dbcan.phy <- merge_phyloseq(otu, tax, sample_data(meta))

# Remove singletons
dbcan.1 <- prune_taxa(taxa_sums(dbcan.phy) > 1, dbcan.phy)
  # Removes 16 families
dbcan.1 <- transform_sample_counts(dbcan.1, function(x){(x/sum(x))*100})

# Remove 5-tons
dbcan.5 <- prune_taxa(taxa_sums(dbcan.phy) > 5, dbcan.phy)
  # Removes 59 families
dbcan.5 <- transform_sample_counts(dbcan.5, function(x){(x/sum(x))*100})

# Remove 10-tons
dbcan.10 <- prune_taxa(taxa_sums(dbcan.phy) > 10, dbcan.phy)
  # Removes 72 families
dbcan.10 <- transform_sample_counts(dbcan.10, function(x){(x/sum(x))*100})

# Remove 50-tons
dbcan.50 <- prune_taxa(taxa_sums(dbcan.phy) > 50, dbcan.phy)
  # Removes 132 families
dbcan.50 <- transform_sample_counts(dbcan.50, function(x){(x/sum(x))*100})

# Remove 100-tons
dbcan.100 <- prune_taxa(taxa_sums(dbcan.phy) > 100, dbcan.phy)
  # Removes 177 families
dbcan.100 <- transform_sample_counts(dbcan.100, function(x){(x/sum(x))*100})

# Calculate rel abund
dbcan99 <- transform_sample_counts(dbcan.phy, function(x){(x/sum(x))*100})

# Remove families w/ < 0.01% avg rel abund
dbcan99 <- prune_taxa(((taxa_sums(dbcan99)) / nsamples(dbcan99)) > 0.01, dbcan99)
...
```{r}
# Families w/ avg rel abund > 0.1%
dbcan.nmds <- ordinate(physeq = dbcan99, method = "NMDS", distance = "bray")
  # stress = 0.02657713
plot_ordination(physeq = dbcan99, ordination = dbcan.nmds, color = "Season")


# Removed singletons
dbcan.1.nmds <- ordinate(physeq = dbcan.1, method = "NMDS", distance = "bray")
  # stress = 0.06448245 
plot_ordination(physeq = dbcan.1, ordination = dbcan.1.nmds, color = "Season")


# Removed 5-tons
dbcan.5.nmds <- ordinate(physeq = dbcan.5, method = "NMDS", distance = "bray")
  # stress = 0.09369415
plot_ordination(physeq = dbcan.5, ordination = dbcan.5.nmds, color = "Season")


# Removed 10-tons
dbcan.10.nmds <- ordinate(physeq = dbcan.10, method = "NMDS", distance = "bray")
  # stress = 0.05952689 
plot_ordination(physeq = dbcan.10, ordination = dbcan.10.nmds, color = "Season")


# Removed 50-tons
dbcan.50.nmds <- ordinate(physeq = dbcan.50, method = "NMDS", distance = "bray")
  # stress = 0.09460896
plot_ordination(physeq = dbcan.50, ordination = dbcan.50.nmds, color = "Season")

# Removed 100-tons
dbcan.100.nmds <- ordinate(physeq = dbcan.100, method = "NMDS", distance = "bray")
  # stress = 0.02244543
plot_ordination(physeq = dbcan.100, ordination = dbcan.100.nmds, color = "Season")
```

### dbCAN - Family Bray-Curtis PCoA
```{r}
# Families w/ avg rel abund > 0.1%
dbcan.pcoa <- ordinate(physeq = dbcan99, method="PCoA", distance="bray")
plot_ordination(physeq = dbcan99, ordination = dbcan.pcoa, axes = c(1,2),
              color = "Season")
  # Axis 1 + 2 = 72.6%
...
```{r}
### PERMANOVA
dbcan.bray <- phyloseq::distance(dbcan99, method="bray")
...
### Pairwise PERMANOVA
pairwise.adonis(as.matrix(t(otu_table(dbcan99))), meta$Season, sim.method="bray", p.adjust.m="fdr")
  # Summer-Winter: adj-pval=0.1, R2 = 0.4589260
  # Summer-Spring: adj-pval=0.1, R2 = 0.3684097
  # Winter-Spring: adj-pval=0.1, R2 = 0.6064905


### Global PERMANOVA is sign, but not pairwise PERMANOVA-- why?
...
### Removed singletons
dbcan.1.bray <- phyloseq::distance(dbcan.1, method="bray")

# Season
adonis(dbcan.1.bray~Season, data=meta, permutations = 99999)
  # p-value = 0.00738
beta.tax = betadisper(d=dbcan.1.bray, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.659

# Pairwise PERMANOVA
pairwise.adonis(as.matrix(t(otu_table(dbcan.1))), meta$Season, sim.method="bray", p.adjust.m="fdr")
  # Summer-Winter: adj-pval=0.15, R2 = 0.4396081
  # Summer-Spring: adj-pval=0.20, R2 = 0.3469957
  # Winter-Spring: adj-pval=0.15, R2 = 0.5767073



### Removed 5-tons
dbcan.5.bray <- phyloseq::distance(dbcan.5, method="bray")

# Season
adonis(dbcan.5.bray~Season, data=meta, permutations = 99999)
  # p-value = 0.0075
beta.tax = betadisper(d=dbcan.5.bray, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.642

# Pairwise PERMANOVA
pairwise.adonis(as.matrix(t(otu_table(dbcan.5))), meta$Season, sim.method="bray", p.adjust.m="fdr")
  # Summer-Winter: adj-pval=0.15, R2 = 0.4400456
  # Summer-Spring: adj-pval=0.2, R2 = 0.3481719
  # Winter-Spring: adj-pval=0.15, R2 = 0.5777577


### Removed 10-tons
dbcan.10.bray <- phyloseq::distance(dbcan.10, method="bray")

# Season
adonis(dbcan.10.bray~Season, data=meta, permutations = 99999)
  # p-value = 0.00739
beta.tax = betadisper(d=dbcan.10.bray, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.702

# Pairwise PERMANOVA
pairwise.adonis(as.matrix(t(otu_table(dbcan.10))), meta$Season, sim.method="bray", p.adjust.m="fdr")
  # Summer-Winter: adj-pval=0.15, R2 = 0.4406695
  # Summer-Spring: adj-pval=0.20, R2 = 0.3481066
  # Winter-Spring: adj-pval=0.15, R2 = 0.5782242



### Removed 50-tons
dbcan.50.bray <- phyloseq::distance(dbcan.50, method="bray")

# Season
adonis(dbcan.50.bray~Season, data=meta, permutations = 99999)
  # p-value = 0.00698
beta.tax = betadisper(d=dbcan.50.bray, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.632

# Pairwise PERMANOVA
pairwise.adonis(as.matrix(t(otu_table(dbcan.50))), meta$Season, sim.method="bray", p.adjust.m="fdr")
  # Summer-Winter: adj-pval=0.15, R2 = 0.4445745
  # Summer-Spring: adj-pval=0.20, R2 = 0.3570909
  # Winter-Spring: adj-pval=0.15, R2 = 0.5865684


### Removed 100-tons
dbcan.100.bray <- phyloseq::distance(dbcan.100, method="bray")

# Season
adonis(dbcan.100.bray~Season, data=meta, permutations = 99999)
  # p-value = 0.00417
beta.tax = betadisper(d=dbcan.100.bray, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.652

# Pairwise PERMANOVA
pairwise.adonis(as.matrix(t(otu_table(dbcan.100))), meta$Season, sim.method="bray", p.adjust.m="fdr")
  # Summer-Winter: adj-pval=0.1, R2 = 0.4525715
  # Summer-Spring: adj-pval=0.1, R2 = 0.3666621 
  # Winter-Spring: adj-pval=0.1, R2 = 0.5983536
...
dbcan.cor <- cor(data.frame(otu_table(dbcan99)), method = "kendall", use="pairwise")
...
### Prepare physeq objects
# Pull out just OTUs that have rel abund > 0.1% (to ensure the OTU analyses remain consistent)
pick <- rownames(otu_table(dbcan99))
dbcan.pick <- prune_taxa(names(taxa_sums(dbcan.phy)) %in% pick, x=dbcan.phy)

# Subset out pairwise seasons
sw.dbcan <- subset_samples(dbcan.pick, Season != "Spring")
sp.dbcan <- subset_samples(dbcan.pick, Season != "Winter")
wp.dbcan <- subset_samples(dbcan.pick, Season != "Summer")

# For some reason DESeq2 isn't recognizing the different Season factors, so I'm resetting them
sample_data(sw.dbcan)$Season <- c("Summer", "Summer", "Winter", "Winter", "Summer", "Winter")
sample_data(sp.dbcan)$Season <- c("Summer", "Summer", "Spring", "Summer", "Spring", "Spring")
sample_data(wp.dbcan)$Season <- c("Winter", "Winter", "Spring", "Winter", "Spring", "Spring")


### Summer vs. Winter
dseq <- deSEQ(data = sw.dbcan, valuetest= ~Season, alpha=0.05)
sw.sum <- which(dseq$log2FoldChange < 0)
sw.sum.fam <- rownames(dseq[sw.sum,])
  # "GT101" "PL1"   "GH43"  "PL11"  "GH115" "CE8"   "PL9"   "CE12"  "GH10"  "GH39"  "CBM37"
sw.win <- which(dseq$log2FoldChange > 0)
sw.win.fam <- rownames(dseq[sw.win,])
  # "CE9"  "GH84"

### Summer vs. Spring
dseq <- deSEQ(data = sp.dbcan, valuetest= ~Season, alpha=0.05)
sp.spr <- which(dseq$log2FoldChange < 0)
sp.spr.fam <- rownames(dseq[sp.spr,])
  # NA
sp.sum <- which(dseq$log2FoldChange > 0)
sp.sum.fam <- rownames(dseq[sp.sum,])
  # "GT41"  "GT101" "GT32"

### Winter vs. Spring
dseq <- deSEQ(data = wp.dbcan, valuetest= ~Season, alpha=0.05)
wp.spr <- which(dseq$log2FoldChange < 0)
wp.spr.fam <- rownames(dseq[wp.spr,])
  # "GH3"   "PL1"   "GH32"  "GH43"  "GH5"   "GH51"  "GH36"  "GH25"  "GH78"  "GH115" "GH42"  "CE12"  
  # "GH106" "GH10"  "GH39"  "CBM37" "CBM13"
wp.win <- which(dseq$log2FoldChange > 0)
wp.win.fam <- rownames(dseq[wp.win,])
  # "PL22" "GT41" "GH33" "GH20" "GT4"  "GT19" "GH92" "GH16" "GT9"  "GT30" "GH23" "CE11" "GH24" "GH57"
  # "GT3" "GH84"
...
### Prepare physeq objects
# Subset out pairwise seasons
sw.dbcan <- subset_samples(dbcan.1, Season != "Spring")
sp.dbcan <- subset_samples(dbcan.1, Season != "Winter")
wp.dbcan <- subset_samples(dbcan.1, Season != "Summer")
...
sw.sum <- which(dseq$log2FoldChange < 0)
sw.sum.fam <- rownames(dseq[sw.sum,])
  # "GT101" "PL1"   "GH43"  "PL11"  "GH115" "PL9"   "CE12"  "GH10"  "CBM37"
sw.win <- which(dseq$log2FoldChange > 0)
sw.win.fam <- rownames(dseq[sw.win,])
  # "GH84"
...
wp.spr <- which(dseq$log2FoldChange < 0)
wp.spr.fam <- rownames(dseq[wp.spr,])
  # "GH3"   "PL1"   "GH32"  "GH43"  "GH5"   "GH51"  "GH36"  "GH25"  "GH78"  "GH115" "GH42"  "GH129"
  # "CE12"  "GH106" "GH10"  "GH39"  "CBM37" "GH59"  "CBM13" "GH14"  "CBM2" 
wp.win <- which(dseq$log2FoldChange > 0)
wp.win.fam <- rownames(dseq[wp.win,])
  # "PL22" "GT41" "GH20" "GT4"  "GH92" "GT9"  "GT30" "CE11" "GH57" "GT3"  "GH84"
...
### Prepare physeq objects
# Subset out pairwise seasons
sw.dbcan <- subset_samples(dbcan.5, Season != "Spring")
sp.dbcan <- subset_samples(dbcan.5, Season != "Winter")
wp.dbcan <- subset_samples(dbcan.5, Season != "Summer")
...
sw.sum <- which(dseq$log2FoldChange < 0)
sw.sum.fam <- rownames(dseq[sw.sum,])
  # "GT101" "PL1"   "GH43"  "GH115" "PL9"   "CE12"  "GH10"  "CBM37"
sw.win <- which(dseq$log2FoldChange > 0)
sw.win.fam <- rownames(dseq[sw.win,])
  # NA
...
wp.spr <- which(dseq$log2FoldChange < 0)
wp.spr.fam <- rownames(dseq[wp.spr,])
  # "GH3"   "GH31"  "PL1"   "GH32"  "GH43"  "GH5"   "GH51"  "GH36"  "GH25"  "GH78"  "GH115" "GH42"
  # "GH129" "CE12"  "GH106" "GH10"  "GH39"  "CBM37" "GH59"  "CBM13" "GH14"  "CBM2" 
wp.win <- which(dseq$log2FoldChange > 0)
wp.win.fam <- rownames(dseq[wp.win,])
  # "PL22" "GT41" "GH20" "GT4"  "GH92" "GT9"  "GT30" "CE11" "GH57" "GT3"  "GH84"
...
### Prepare physeq objects
# Subset out pairwise seasons
sw.dbcan <- subset_samples(dbcan.10, Season != "Spring")
sp.dbcan <- subset_samples(dbcan.10, Season != "Winter")
wp.dbcan <- subset_samples(dbcan.10, Season != "Summer")
...
### Prepare physeq objects
# Subset out pairwise seasons
sw.dbcan <- subset_samples(dbcan.50, Season != "Spring")
sp.dbcan <- subset_samples(dbcan.50, Season != "Winter")
wp.dbcan <- subset_samples(dbcan.50, Season != "Summer")
...
sw.sum <- which(dseq$log2FoldChange < 0)
sw.sum.fam <- rownames(dseq[sw.sum,])
  # "GT101" "PL1"   "GH43"  "PL11"  "GH115" "PL9"   "CE12"  "GH10"  "GH39"  "CBM37"
sw.win <- which(dseq$log2FoldChange > 0)
sw.win.fam <- rownames(dseq[sw.win,])
  # "CE9"  "GH84"
...
wp.spr.fam <- rownames(dseq[wp.spr,])
  # "GH3"   "PL1"   "GH32"  "GH43"  "GH5"   "GH51"  "GH36"  "GH25"  "GH78"  "GH115" "GH42"  "GH129"
  # "CE12"  "GH106" "GH10"  "GH39"  "CBM37" "CBM13" "CBM34" "CBM2" 
wp.win <- which(dseq$log2FoldChange > 0)
wp.win.fam <- rownames(dseq[wp.win,])
  # "PL22" "GT41" "GH20" "GT4"  "GH92" "GT9"  "GT30" "CE11" "GH57" "GT3"  "GH84"
...
### Prepare physeq objects
# Subset out pairwise seasons
sw.dbcan <- subset_samples(dbcan.100, Season != "Spring")
sp.dbcan <- subset_samples(dbcan.100, Season != "Winter")
wp.dbcan <- subset_samples(dbcan.100, Season != "Summer")
...
wp.spr.fam <- rownames(dseq[wp.spr,])
  # "GH3"   "PL1"   "GH32"  "GH43"  "GH5"   "GH51"  "GH36"  "GH25"  "GH78"  "GH115" "GH42"  "GH129"
  # "CE12"  "GH106" "GH10"  "GH39"  "CBM37" "CBM13" "CBM34"
wp.win <- which(dseq$log2FoldChange > 0)
wp.win.fam <- rownames(dseq[wp.win,])
  # "PL22" "GT41" "GH33" "GH20" "GT4"  "GT19" "GH92" "GH16" "GT9"  "GT30" "GH23" "CE11" "GH24" "GH57"
  # "GT3"  "GH84"
