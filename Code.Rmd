...
source('scripts/import.phylum.R')
...
phy.sum.sum <- phy.sum %>%
  group_by(Phylum) %>%
  summarize(Total_Mean = mean(Total_Mean),
            Classified_Mean = mean(Classified_Mean))
...
for (i in 1:length(top.total)){
  ...
  for (j in 1:length(top)){
    phy.total$Season <- phy.sum$Season[top[j]]
    phy.total$Phylum <- 
  }
  phy.total <- 
}
...
phy.total.trim <- phy.sum[which(phy.sum$Total_Mean > 0.01), 1:5]
phy.clas.trim <- phy.sum[which(phy.sum$Classified_Mean > 0.01),]
phy.clas.trim <- phy.clas.trim[,-3:-5]
phy.clas.trim <- phy.clas.trim[-which(phy.clas.trim$Phylum == "Unclassified"),]
phy.total.trim$Phylum <- ordered(phy.total.trim$Phylum, levels=c("Unclassified", "Bacteroidetes", "Firmicutes", "Verrucomicrobia", "Proteobacteria", "Actinobacteria", "Candidatus Melainabacteria", "Spirochaetes", "Tenericutes", "Elusimicrobia", "Fusobacteria", "Preaxostyla", "Chloroflexi", "Fibrobacteres", "Synergistetes", "Virus", "Planctomycetes", "Euryarchaeota"))
phy.clas.trim$Phylum <- ordered(phy.clas.trim$Phylum, levels=c("Bacteroidetes", "Firmicutes", "Verrucomicrobia", "Proteobacteria", "Actinobacteria", "Candidatus Melainabacteria", "Spirochaetes", "Tenericutes", "Elusimicrobia", "Fusobacteria", "Preaxostyla", "Chloroflexi", "Fibrobacteres", "Synergistetes", "Virus", "Planctomycetes", "Euryarchaeota", "Parabasalia", "Cyanobacteria", "Lentisphaerae", "Acidobacteria"))


# Barplot for all sequences
ggplot(phy.total.trim, aes(x=Phylum, y=Total_Mean, fill=Phylum)) +
  geom_bar(stat="identity") +
  facet_grid(Season ~ .) +
  geom_errorbar(aes(ymax=Total_Mean+Total_SE, ymin=Total_Mean-Total_SD), width=0.25) +
  scale_y_continuous(limits=c(-0.5,60)) +
  facet_grid(Season ~ .) +
  xlab("Phyla") +
  ylab("Mean Relative Abundance") +
