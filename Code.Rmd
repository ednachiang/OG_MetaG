...
fam.tax <- tax_table(class)
row.names(fam.tax) <- row.names(class)
...
### dbCAN Jaccard
```{r}
jac <- ordinate(physeq=fam.physeq, method="NMDS", distance="jaccard")
  # stress = 0.08480145
plot_ordination(physeq=fam.physeq, ordination=jac, color="Season")
plot_ordination(physeq=fam.physeq, ordination=jac, color="Sex")
...
```{r}
### Separate out pairw
simper.pretty(fam.norm, metrics = meta, interesting = 'Season', perc_cutoff = 0.5, low_cutoff = 'y', low_val = 0.01, 'dbcan.fam')

fam.simper <- read.csv("dbcan.fam_clean_simper.csv")

kruskal.pretty(otu = fam.norm, metrics = meta, csv =  fam.simper, interesting = 'Season', output_name = 'fam.kw')

fam.simper.kw <- read.csv("fam.kw_krusk_simper.csv")
```
...
class <- read.csv('COG_cog_classification.csv')
row.names(class) <- class[,1]
#class <- class[,-1]
...
rownames(cog.class) <- cog.class[,1]
...
sum.cog <- rownames(test[sum,])
  # "COG2207" "COG3507" "COG3693" "COG3866" "COG4677" "COG5021" "COG5434"
win <- which(test$log2FoldChange > 0)
...
spr.cog <- rownames(test[spr,])
  # "COG0183" "COG0395" "COG0411" "COG0444" "COG0448" "COG0559" "COG1070" "COG1080" "COG1126" "COG1129" "COG1172" "COG1173" "COG1175" "COG1307" "COG1609" "COG1744" "COG1775" "COG1802" "COG1879" "COG2972" "COG3077" "COG3958" "COG3959" "COG4166" "COG4608" "COG4905"
sum <- which(test$log2FoldChange > 0)
sum.cog <- rownames(test[sum,])
...
test <- deSEQ(data=sw.physeq, valuetest= ~SubSeason)
act <- which(test$log2FoldChange < 0)
act.cog <- rownames(test[act,])
  # "COG0004" "COG0028" "COG0119" "COG0235" "COG0246" "COG0372" "COG0395" "COG0407" "COG0448" "COG0456" "COG0554" "COG0561" "COG0675" "COG0722" "COG0745" "COG0855" "COG1026" "COG1051" "COG1061" "COG1070" "COG1104" "COG1105" "COG1122" "COG1131" "COG1132" "COG1134" "COG1136" "COG1161" "COG1164" "COG1175" "COG1191" "COG1196" "COG1302" "COG1307" "COG1349" "COG1453" "COG1461" "COG1472" "COG1486" "COG1501" "COG1511" "COG1609" "COG1621" "COG1653" "COG1662" "COG1682" "COG1686" "COG1874" "COG1882" "COG1918" "COG1925" "COG1959" "COG2002" "COG2081" "COG2176" "COG2182" "COG2190" "COG2199" "COG2200" "COG2207" "COG2252" "COG2333" "COG2357" "COG2407" "COG2508" "COG2730" "COG2816" "COG2972" "COG3044" "COG3153" "COG3279" "COG3331" "COG3345" "COG3437" "COG3459" "COG3464" "COG3481" "COG3507" "COG3534" "COG3635" "COG3664" "COG3693" "COG3757" "COG3833" "COG3835" "COG3839" "COG3857" "COG3887" "COG3894" "COG3940" "COG3958" "COG3959" "COG3981" "COG4166" "COG4189" "COG4209" "COG4284" "COG4465" "COG4468" "COG4485" "COG4509" "COG4626" "COG4709" "COG4753" "COG4805" "COG4870" "COG4962" "COG5023" "COG5412" "COG5578"
hib <- which(test$log2FoldChange > 0)
hib.cog <- rownames(test[hib,])
  # "COG0156" "COG0457" "COG0612" "COG0760" "COG0767" "COG0795" "COG0810" "COG0811" "COG0841" "COG0845" "COG0848" "COG1043" "COG1409" "COG1452" "COG1463" "COG1538" "COG1596" "COG1629" "COG1663" "COG2885" "COG2982" "COG3176" "COG3203" "COG3468" "COG3525" "COG3637" "COG3712" "COG4566" "COG4775" "COG4783"

### Identify Overrep COGs
cog.act.id <- data.frame(act.cog)
cog.act.id$ID <- NA
for(i in 1:length(act.cog)){
  test <- which(act.cog[i] == cog.3715[,1])
  cog.act.id[i,2] <- cog.3715[test,2]
}
#write.csv(cog.act.id, 'COG/cog.act.id.csv')
cog.hib.id <- data.frame(hib.cog)
for(i in 1:length(hib.cog)){
  test <- which(hib.cog[i] == cog.3723[,1])
  cog.hib.id[i,2] <- cog.3723[test,2]
}
#write.csv(cog.hib.id, 'COG/cog.hib.id.csv')
```
...
row.names(cog.class) <- cog.class[,1]
#class <- class[,-1]
...
plot_ordination(physeq=cog.physeq, ordination=nmds.cog, color="SubSeason")
...
## Diet
adonis(cog.bray~ SubSeason, data=cog.sampdf)
  # p-value = 0.237
beta.tax = betadisper(d=cog.bray, group=cog.sampdf$SubSeason)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.426
...
anosim(cog.bray, meta$Season, permutations=1000)
  # p-value = 0.1852
anosim(cog.bray, meta$SubSeason, permutations=1000)
  # p-value = 0.1049
```
...
plot_ordination(physeq=cog.physeq, ordination=jac.cog, color="SubSeason")
...
## Diet
adonis(cog.jac~ SubSeason, data=cog.sampdf)
  # p-value = 0.17
beta.tax = betadisper(d=cog.jac, group=cog.sampdf$SubSeason)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.398
...
```{r}
### Separate out pairw
simper(cog.all.norm, group= meta$SubSeason)


simper.pretty(cog.all.norm, metrics = meta, interesting = 'Season', perc_cutoff = 0.5, low_cutoff = 'y', low_val = 0.01, 'cog')

cog.simper <- read.csv("cog_clean_simper.csv")

kruskal.pretty(otu = cog.all.norm, metrics = meta, csv = cog.simper, interesting = 'Season', output_name = 'cog.kw')
