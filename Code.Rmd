...
### dbCAN - Compare total normalized CAZyme counts
```{r}
# Summarize data
dbcan.df <- psmelt(dbcan.fam.norm.phy)
dbcan.sum <- dbcan.df %>%
  group_by(Sample, Season) %>%
  summarize(Sum = sum(Abundance))
dbcan.sum$Season <- ordered(dbcan.sum$Season, levels=c("Summer", "Winter", "Spring"))

## Dot plot
#tiff("dbcan.normCount.tiff", width=5, height=4, units="in", res= 300)
ggplot(dbcan.sum, aes(x=Season, y=Sum, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1, stackratio=1.15) +
  scale_y_continuous(limits=c(14900,17100)) +
  scale_fill_manual(name="Season", values=c("darkgoldenrod2", "dodgerblue3", "green3")) +
  xlab("Season") +
  ylab("Total Normalized CAZyme Counts") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.background = element_blank(),
        strip.text = element_text(face="bold"),
...
# Test normality
hist(dbcan.sum$Sum)
qqnorm(dbcan.sum$Sum)
qqline(dbcan.sum$Sum)
shapiro.test(dbcan.sum$Sum)
  # p-value = 0.9777
  # Normal

summary(aov(Sum ~ Season, data=dbcan.sum))
  # p-value = 0.263
...
# NormTransform
dbcan.ntd <- normTransform(dbcan.dds)
dbcan.heatmap.phy <- merge_phyloseq(otu_table(assay(dbcan.ntd), taxa_are_rows=T), sample_data(dbcan.fam.norm.phy), tax_table(dbcan.fam.norm.phy))
...
dbcan.sig <- subset_taxa(dbcan.heatmap.phy, Family == "GT101" | Family == "GH59" | Family == "PL1" |
                           ...
                           Family == "GH32" | Family == "GH39" | Family == "GH42" | Family == "GH78" |
                           Family == "GH106" | Family == "GH20" | Family == "GH84" | Family == "GT3" |
                           Family == "GH9" | Family == "CE11" | Family == "GH43" | Family == "GH3" |
                           Family == "GT4" | Family == "GT41")


plot_heatmap(dbcan.sig, sample.label = "Season", sample.order = c("3715", "3717", "3744", "3723", "3733", "3772", "3734", "3773", "3775"), taxa.order = c("GT41", "GT4", "GH3", "GH43", "CE11", "GH9", "GT3", "GH84", "GH20", "GH106", "GH78", "GH42", "GH39", "GH32", "GH5", "CE12", "GH115", "GH10", "PL1", "GH59", "GT101")) +
  geom_tile(colour="gray40") +
  scale_x_discrete(position = "top", labels=c("Summer", "Summer", "Summer", "Winter", "Winter", "Winter", "Spring", "Spring", "Spring")) +
  xlab("Season\n\n") +
  ylab("CAZyme Family\n\n") +
...
### dbCAN - DESeq 
### dbCAN - Testing DESeq plots
```{r}
# https://www.bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#exploring-and-exporting-results

# MA plot (log ratio (M) vs. average (A))
plotMA(dbcan.dds, alpha = 0.05)

# Plot single gene normalized counts
plotCounts(dbcan.dds, gene="GH20", intgroup="Season")

# Heatmap of count matrix
select <- order(rowMeans(counts(dbcan.dds, normalized=T)), decreasing=T)[1:20]
df <- as.data.frame(colData(dbcan.dds)[,"Season"])
pheatmap(assay(normTransform(dbcan.dds)[select,], cluster_rows=F, show_rownames=F, cluster_cols=F, annotation_col=df))

# Heatmap of count matrix (variance stabilizing transformed)
vsd <- varianceStabilizingTransformation(dbcan.dds, blind=F)
pheatmap(assay(vsd)[select,])

# Heatmap of count matrix (rlog transformed)
rld <- rlog(dbcan.dds, blind=F)
pheatmap(assay(rld)[select,])

# Heatmap of sample-to-sample distances
sampleDists <- dist(t(assay(vsd)))
sampleDistMatrix <- as.matrix(sampleDists)
rownames(sampleDistMatrix) <- paste(vsd$Season, vsd$type, sep="-")
...
# PCA 
plotPCA(vsd, intgroup=c("Season"))

# Customized PCA
pcaData <- plotPCA(vsd, intgroup=c("Season"), returnData=TRUE)
percentVar <- round(100 * attr(pcaData, "percentVar"))
ggplot(pcaData, aes(PC1, PC2, color=Season)) +
  geom_point(size=3) +
...
```
### Mucin - Compare total normalized mucin-deg counts
```{r}
...
#tiff("mucin.normCount.tiff", width=5, height=4, units="in", res= 300)
ggplot(mucin.sum, aes(x=Season, y=Sum, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1, stackratio=1.15) +
  scale_y_continuous(limits=c(324,436)) +
  scale_fill_manual(name="Season", values=c("darkgoldenrod2", "dodgerblue3", "green3")) +
  xlab("Season") +
  ylab("Total Normalized Mucin-Deg Counts") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        strip.background = element_blank(),
        strip.text = element_text(face="bold"),
...
# Test normality
hist(mucin.sum$Sum)
qqnorm(mucin.sum$Sum)
qqline(mucin.sum$Sum)
shapiro.test(mucin.sum$Sum)
  # p-value = 0.2094
  # Normal

summary(aov(Sum ~ Season, data=mucin.sum))
  # p-value = 0.0252
TukeyHSD(aov(Sum ~ Season, data=mucin.sum))
  # Winter - Summer = 0.9990301
  # Spring - Summer = 0.0389443*
  # Spring - Winter = 0.0370233*
```
