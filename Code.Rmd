...
## dbCAN - Family Jaccard NMDS
```{r}
dbcan.jac.nmds <- ordinate(physeq=dbcan99, method="NMDS", distance="jaccard")
  # stress = 0.02657695

plot_ordination(physeq=dbcan99, ordination=dbcan.jac.nmds, color="Season")
```


### dbCAN - Family Jaccard PCoA
```{r}
# Families w/ avg rel abund > 0.1%
dbcan.jac.pcoa <- ordinate(physeq = dbcan99, method="PCoA", distance="jaccard")
plot_ordination(physeq = dbcan99, ordination = dbcan.jac.pcoa, axes = c(1,2),
              color = "Season")
  # Axis 1 + 2 = 68.5
  # Bray PCoA explains more variability than Jaccard PCoA
```


### dbCAN - Family Jaccard PERMANOVA
```{r}
### PERMANOVA
dbcan.jac <- phyloseq::distance(dbcan99, method="jaccard")

# Season
adonis(dbcan.jac~Season, data=meta, permutations = 99999)
  # p-value = 0.00356
beta.tax = betadisper(d=dbcan.jac, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.594
...
# Diet
adonis(dbcan.jac~Diet, data=meta, permutations = 99999)
  # p-value = 0.01215
beta.tax = betadisper(d=dbcan.jac, group=meta$Diet)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.113
...
# Sex
adonis(dbcan.jac~Sex, data=meta, permutations = 99999)
  # p-value = 0.4288
beta.tax = betadisper(d=dbcan.jac, group=meta$Sex)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.779
...
### Pairwise PERMANOVA
pairwise.adonis(as.matrix(t(otu_table(dbcan99))), meta$Season, sim.method="jaccard", p.adjust.m="fdr")
  # Summer-Winter: adj-pval=0.1, R2 = 0.4432689
  # Summer-Spring: adj-pval=0.1, R2 = 0.3477866
  # Winter-Spring: adj-pval=0.1, R2 = 0.5800833
