...
##### Test removing CAZymes w/ normalized count < 3
...
cut.phy <- merge_phyloseq(otu_table(dbcan.norm, taxa_are_rows=T), sample_data(dbcan.phy), tax_table(dbcan.phy))
cut.phy <- prune_taxa(taxa_sums(cut.phy) > 2, cut.phy)
dbcan.fam.norm <- cut.phy
...
dbcan.dds <- phyloseq_to_deseq2(dbcan.phy, ~Season)
  # Design variable must be unordered otherwise you get an error
dbcan.dds$Season <- factor(dbcan.dds$Season, levels=c("Summer", "Winter", "Spring"))
dbcan.dds <- dbcan.dds[rowSums(counts(dbcan.dds)) > 2,]
