...
# Remove families w/ < 0.1% avg rel abund
#dbcan99 <- prune_taxa(((taxa_sums(dbcan99)) / nsamples(dbcan99)) > 0.1, dbcan99)
...
sum <- which(dseq$log2FoldChange < 0)
sum.fam <- rownames(dseq[sum,])
win <- which(dseq$log2GoldChange > 0)
win.fam <- rownames(dseq[win,])
```
