...
for (i in 1:length(top.total)){
  ...
  for (j in 1:length(top)){
    ...
    phy.total$Mean[(j + 3*(i-1))] <- phy.sum$Total_Mean[top[j]]
    phy.total$SD[(j + 3*(i-1))] <- phy.sum$Total_SD[top[j]]
    phy.total$SE[(j + 3*(i-1))] <- phy.sum$Total_SE[top[j]]
  }
}
...
for (i in 1:length(clas.total)){
  ...
  for (j in 1:length(top)){
    ...
    phy.clas.total$Mean[(j + 3*(i-1))] <- phy.sum$Total_Mean[top[j]]
    phy.clas.total$SD[(j + 3*(i-1))] <- phy.sum$Total_SD[top[j]]
    phy.clas.total$SE[(j + 3*(i-1))] <- phy.sum$Total_SE[top[j]]
  }
}
