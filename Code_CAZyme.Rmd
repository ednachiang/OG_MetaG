...
# log2(n+1)
dbcan.transform <- normTransform(dbcan.dds)
```
...
dbcan.sig.transform <- data.frame(assay(dbcan.transform))
dbcan.sig.transform <- otu_table(dbcan.sig.transform, taxa_are_rows = T)
colnames(dbcan.sig.transform) <- c("3715", "3717", "3723", "3733", "3734", "3744", "3772",
                             "3773" , "3775")
dbcan.sig <- merge_phyloseq(dbcan.sig.transform, tax_table(dbcan.phy),
...
plot_heatmap(dbcan.sig, sample.label = "Season", sample.order = c("3715", "3717", "3744", "3723", "3733", "3772", "3734", "3773", "3775"), taxa.order = c("CE11", "GT41", "GT9", "GT4", "GT3", "GH84", "GH20", "GH106", "GH78", "GH42", "GH39", "GH32", "GH5", "GH3", "CE12", "GH115", "GH43", "GH10", "PL1", "GT101")) +
  ...
  xlab("Season\n\n") +
  ylab("CAZyme Family\n\n") +
  scale_fill_gradientn(name=expression("Transformed\nNormalized\nCounts\n"), limits = c(3,10.2),
...
dbcan.dist <- Dist(t(assay(dbcan.transform)), method = "kendall")
dbcan.dist.matrix <- as.matrix(dbcan.dist)
rownames(dbcan.dist.matrix) <- dbcan.transform$Season
colnames(dbcan.dist.matrix) <- dbcan.transform$Season
...
```{r}
dbcan.pca.data <- plotPCA(dbcan.transform, intgroup="Season", returnData=T)
dbcan.pca.perc <- round(100*attr(dbcan.pca.data, "percentVar"))
# PCA variance explained (total) = 53%
...
# log2(n+1)
muc.transform <- normTransform(mucin.dds)
```
...
```{r}
muc.sig.transform <- data.frame(assay(muc.transform))
muc.sig.transform <- otu_table(muc.sig.transform, taxa_are_rows = T)
colnames(muc.sig.transform) <- c("3715", "3717", "3723", "3733", "3734", "3744",
                                "3772", "3773" , "3775")
muc.sig <- merge_phyloseq(muc.sig.transform, tax_table(mucin.phy),
...
plot_heatmap(mucin.sig, sample.label = "Season", sample.order = c("3715", "3717", "3744", "3723", "3733", "3772", "3734", "3773", "3775"), taxa.order = c("GH110_3.2.1.22", "GH84_3.2.1.169", "GH42_3.2.1.23", "GH36_3.2.1.22", "GH112_2.4.1.211")) +
  ...
  scale_fill_gradientn(name=expression("Transformed\nNormalized\nCounts\n"), limits=c(2.5, 7.0),
                       breaks=c(2.5, 4, 5.5, 7.0), 
                       labels=c("2.5", "4", "5.5", "7.0"),
                       colours = c("#FFFFFF", "#ffffcc", "#ffeda0", "#fed976",
                                   "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c",
                                   "#bd0026", "#800026", "#33000f")) +
  theme(axis.text.x = element_blank(),
...
muc.dist <- Dist(t(assay(muc.transform)), method = "kendall")
...
```{r}
muc.pca.data <- plotPCA(muc.transform, intgroup="Season", returnData=T)
muc.pca.perc <- round(100*attr(muc.pca.data, "percentVar"))
# PCA total variance explained = 67%
