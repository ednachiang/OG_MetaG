...
dbcan.class.phy <- transform_sample_counts(dbcan.class.phy, function(x){(x/sum(x))*100})

# Summarize data
dbcan.class.df <- psmelt(dbcan.class.phy)
dbcan.class.df$Class <- ordered(dbcan.class.df$Class, c("Glycoside Hydrolase",
                                                                  "Glycosyltransferase",
                                                                  "Carbohydrate Esterase",
                                                                  "Polysaccharide Lyase"))
dbcan.class.df$Season <- ordered(dbcan.class.df$Season, levels = c("Summer", "Winter", "Spring"))
...
# Plot barplot
#tiff("dbcan.class.tiff", width=11, height=4, units="in", res= 300)
ggplot(dbcan.class.df, aes(x=Season, y=Abundance, fill=Season)) +
...
test <- transform_sample_counts(dbcan.fam.norm.phy, function(x){(x/sum(x))*100})
dbcan.hell.otu <- otu_table(test)
...
### dbCAN - DESeq heatmap of normalized + transformed counts FOR AAAS
```{r}
# Prepare data
dbcan.hell.otu <- otu_table(t(dbcan.hell), taxa_are_rows = T)
# Trying it with rel abund
...
dbcan.sig <- merge_phyloseq(dbcan.hell.otu, tax_table(dbcan.phy),
                            sample_data(dbcan.phy))
dbcan.act.sig <- subset_taxa(dbcan.sig, Family == "PL1" | Family == "GH10" | 
                               Family == "GH43" | Family == "GH115" | Family == "CE12")
dbcan.win.sig <- subset_taxa(dbcan.sig, Family == "GH20" | Family == "GH84" | 
                               Family == "GT4" | Family == "GT9" | Family == "CE11")


#tiff("aaas.dbcan.active.heatmap.tiff", width=7, height=3, units="in", res=300)
plot_heatmap(dbcan.act.sig, sample.label = "Season", sample.order = c("3715", "3717", "3744", "3723", "3733", "3772", "3734", "3773", "3775"), taxa.order = c("CE12", "GH115", "GH43", "GH10", "PL1")) +
  geom_tile(colour="gray40") +
  scale_x_discrete(position = "top", labels=c("Summer", "Summer", "Summer", "Winter", "Winter", "Winter", "Spring", "Spring", "Spring")) +
  xlab("Season\n\n") +
  ylab("CAZyme Family\n\n") +
...
#tiff("aaas.dbcan.winter.heatmap.tiff", width=7, height=3, units="in", res=300)
plot_heatmap(dbcan.win.sig, sample.label = "Season", sample.order = c("3715", "3717", "3744", "3723", "3733", "3772", "3734", "3773", "3775"), taxa.order = c("CE11", "GT9", "GT4", "GH84", "GH20")) +
  geom_tile(colour="gray40") +
  scale_x_discrete(position = "top", labels=c("Summer", "Summer", "Summer", "Winter", "Winter", "Winter", "Spring", "Spring", "Spring")) +
  xlab("Season\n\n") +
  ylab("CAZyme Family\n\n") +
  scale_fill_gradientn(name=expression("Hellinger-\nTransformed\nNormalized\nCounts"), 
                       limits = c(0.027, 0.270),
                       #breaks = c(0.02, 0.07, 0.12, 0.17, 0.22, 0.27),
                       #labels = c("0.02", "0.07", "0.12", "0.17", "0.22", "0.27"),
                       colours = c("#FFFFFF", "#ffffcc", "#ffeda0", "#fed976",
                                   "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c",
...
tiff("aaas.dbcan.pcoa.tiff", width=8, height=6, units="in", res=300)
...
rownames(muc.dist.matrix) <- meta$Season
colnames(muc.dist.matrix) <- meta$Season
