...
### COG - Variance stabilizing transformation
```{r}
a <- as.numeric(cog.norm[,1])
b <- as.numeric(cog.norm[,2])
c <- as.numeric(cog.norm[,3])
d <- as.numeric(cog.norm[,4])
e <- as.numeric(cog.norm[,5])
f <- as.numeric(cog.norm[,6])
g <- as.numeric(cog.norm[,7])
h <- as.numeric(cog.norm[,8])
i <- as.numeric(cog.norm[,9])
cog.num <- c(a,b,c,d,e,f,g,h,i)
cog.num <- na.omit(cog.num)
descdist(test, discrete = F, boot=1000)
  # min:  0   max:  17.58868 
  # median:  0.1220457 
  # mean:  0.4213211 
  # estimated sd:  0.797082 
  # estimated skewness:  6.432063 
  # estimated kurtosis:  75.04978 

test <- cog.num[-which(cog.num == 0)]
fw <- fitdist(test, "weibull")
fg <- fitdist(test, "gamma")
fe <- fitdist(test, "exp")
...
# Levene test
test <- psmelt(cog.df)
levene.test(cog.df[,1:3872], group = "Season")
```

### COG - Clustering - Count-level
```{r}
...
# Heatmap
pheatmap(cog.dist.matrix, clustering_distance_rows = cog.dist, 
         clustering_distance_cols = cog.dist, col = c("#FFFFFF", "#ffffcc", "#ffeda0", "#fed976", "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c", "#bd0026", "#800026"))

# Dendrogram
cog.hclust <- hclust(cog.dist)
...
colnames(cog) <- c("3715", "3717", "3723", "3733", "3734", "3744", "3772", 
                        "3773", "3775")
cog.distk <- Dist(t(cog), method = "kendall")
cog.distk.matrix <- as.matrix(cog.distk)
rownames(cog.distk.matrix) <- c("Summer", "Summer", "Winter", "Winter", "Spring", "Summer", "Winter", "Spring", "Spring")
colnames(cog.distk.matrix) <- c("Summer", "Summer", "Winter", "Winter", "Spring", "Summer", "Winter", "Spring", "Spring")

# Heatmap
pheatmap(cog.distk.matrix, clustering_distance_rows = cog.dist, 
         clustering_distance_cols = cog.distk, col = c("#FFFFFF", "#ffffcc", "#ffeda0", "#fed976", "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c", "#bd0026", "#800026"))

# Dendrogram
cog.hclustk <- hclust(cog.distk)
cog.denk <- as.dendrogram(cog.hclustk)
cog.denk.data <- dendro_data(cog.denk, type = "rectangle")
cog.denk.data$labels[,3] <- c("Summer", "Winter", "Winter", "Winter", "Summer", "Spring", "Spring", "Spring", "Summer")
cog.denk.data$labels[,3] <- ordered(cog.denk.data$labels[,3], levels=c("Summer", "Winter", "Spring"))


#tiff("dbcan.dendro.tiff", width=10, height=5, units="in", res=300)
ggplot(cog.denk.data$segments) +
  geom_segment(aes(x=x, y=y, xend=xend, yend=yend)) +
  geom_text(data = cog.den.data$labels, aes(x, y, label=label, colour=label, fontface = "bold"),
            hjust=0, size=5) +
...
adonis(cog.distk ~ Season, meta, permutations = 99999)
  # p-value = 0.01811
...
### PCA - Euclidean
cog.pca <- ordinate(physeq = cog.phy, method = "PCoA", distance = "euclidean")

plot_ordination(physeq = cog.phy, ordination = cog.pca, axes = c(1, 2),
                color="Season") + 
  geom_point(size=5) +
...
### PCA - Bray-Curtis
cog.pca.bc <- ordinate(physeq = cog.phy, method = "PCoA", distance = "bray")

plot_ordination(physeq = cog.phy, ordination = cog.pca.bc, axes = c(1, 2),
                color="Season") + 
  geom_point(size=5) +
  scale_color_manual(values=c("darkgoldenrod2", "dodgerblue3", "green3")) +
  ggtitle("Euclidean Distance") +
...
### Run PERMANOVA on DESEq Bray-Curtis
cog.bray <- phyloseq::distance(physeq=cog.phy, method="bray")
adonis(cog.bray ~ Season, data=meta, permutations = 99999)
  # p-value = 0.05678
beta.tax = betadisper(d=cog.bray, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.618
```

### COG - Clustering - Category-level
```{r}
# https://sites.google.com/site/mb3gustame/reference/dissimilarity
cog.cat <- data.frame(otu_table(cog.cat.phy))
colnames(cog.cat) <- c("3715", "3717", "3723", "3733", "3734", "3744", "3772", 
                        "3773", "3775")
cog.cat.dist <- dist(t(cog.cat))
...
# Heatmap
pheatmap(cog.cat.dist.matrix, clustering_distance_rows = cog.cat.dist, 
         clustering_distance_cols = cog.cat.dist, col = c("#FFFFFF", "#ffffcc", "#ffeda0", "#fed976", "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c", "#bd0026", "#800026"))

# Dendrogram
cog.cat.hclust <- hclust(cog.cat.dist)
...
##### Kendall #####
cog.cat.distk <- Dist(t(cog.cat), method = "kendall")
cog.cat.distk.matrix <- as.matrix(cog.cat.distk)
rownames(cog.cat.distk.matrix) <- c("Summer", "Summer", "Winter", "Winter", "Spring", "Summer", "Winter", "Spring", "Spring")
colnames(cog.cat.distk.matrix) <- c("Summer", "Summer", "Winter", "Winter", "Spring", "Summer", "Winter", "Spring", "Spring")

# Heatmap
pheatmap(cog.cat.distk.matrix, clustering_distance_rows = cog.cat.distk, 
         clustering_distance_cols = cog.cat.distk, col = c("#FFFFFF", "#ffffcc", "#ffeda0", "#fed976", "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c", "#bd0026", "#800026"))

# Dendrogram
cog.cat.hclustk <- hclust(cog.cat.distk)
cog.cat.denk <- as.dendrogram(cog.cat.hclustk)
cog.cat.denk.data <- dendro_data(cog.cat.denk, type = "rectangle")
cog.cat.denk.data$labels[,3] <- c("Summer", "Winter", "Winter", "Winter", "Summer", "Spring", "Spring", "Spring", "Summer")
cog.cat.denk.data$labels[,3] <- ordered(cog.cat.denk.data$labels[,3], levels=c("Summer", "Winter", "Spring"))


#tiff("dbcan.dendro.tiff", width=10, height=5, units="in", res=300)
ggplot(cog.cat.denk.data$segments) +
  geom_segment(aes(x=x, y=y, xend=xend, yend=yend)) +
  geom_text(data = cog.cat.den.data$labels, aes(x, y, label=label, colour=label, fontface = "bold"),
            hjust=0, size=5) +
  ylab("Kendall Distance") +
...
adonis(cog.cat.distk ~ Season, meta, permutations = 99999)
  # p-value = 0.00351
...
### PCA - Euclidean
cog.cat.pca <- ordinate(physeq = cog.cat.phy, method = "PCoA", distance = "euclidean")

plot_ordination(physeq = cog.cat.phy, ordination = cog.cat.pca, axes = c(1, 2),
                color="Season") + 
  geom_point(size=5) +
  scale_color_manual(values=c("darkgoldenrod2", "dodgerblue3", "green3")) +
  ggtitle("Euclidean Distance") +
...
### PCA - Bray-Curtis
cog.cat.pca.bc <- ordinate(physeq = cog.cat.phy, method = "PCoA", distance = "bray")

plot_ordination(physeq = cog.cat.phy, ordination = cog.cat.pca.bc, axes = c(1, 2),
                color="Season") + 
  geom_point(size=5) +
  scale_color_manual(values=c("darkgoldenrod2", "dodgerblue3", "green3")) +
  ggtitle("Euclidean Distance") +
...
### Run PERMANOVA on DESEq Bray-Curtis
cog.cat.bray <- phyloseq::distance(physeq=cog.cat.phy, method="bray")
adonis(cog.cat.bray ~ Season, data=meta, permutations = 99999)
  # p-value = 0.5051
beta.tax = betadisper(d=cog.cat.bray, group=meta$Season)
p <- permutest(beta.tax)
p$tab
  # p-value = 0.579
```
