...
# Test normality
hist(as.numeric(cog.usg.avg))
qqnorm(as.numeric(cog.usg.avg))
qqline(as.numeric(cog.usg.avg))
shapiro.test(as.numeric(cog.usg.avg))
  # p-value = 0.9902
  # Normal

# Format for ANOVA
usg.df <- data.frame(t(cog.usg.avg))
usg.df <- t(usg.df)
usg.df$Season <- c("Summer", "Summer", "Winter", "Winter", "Spring", "Summer", "Winter", "Spring", "Spring")
usg.df$Season <- ordered(usg.df$Season, levels=c("Summer", "Winter", "Spring"))
usg.df$Diet <- c("Feeding", "Feeding", "Fasting", "Fasting", "Feeding", "Feeding", "Fasting", "Feeding", "Feeding")
usg.df$Diet <- ordered(usg.df$Diet, levels=c("Feeding", "Fasting"))
colnames(usg.df) <- c("Counts", "Season", "Diet")

# Test w/ ANOVA
summary(aov(Counts ~ Season, data = usg.df))
  # p-value = 0.235
summary(aov(Counts ~ Diet, data = usg.df))
  # p-value = 0.35


### Dot plot
#tiff("cog.usg.tiff", width=5, height=4, units="in", res= 300)
ggplot(usg.df, aes(x=Season, y=Counts, fill=Season)) +
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1, stackratio=1.15) +
  ylab("Number of universal single copy genes") +
  scale_fill_manual(values=c("darkgoldenrod2", "dodgerblue3", "green3")) +
  scale_y_continuous(breaks = c(73, 99, 125, 151, 178, 204), limits=c(73, 204)) +
...
# Identify COGs that appear in < 3 samples
cog.rem <- list()
for(i in 1:nrow(cog.norm)){
  empty <- length(which(cog.norm[i,] == 0))
    # Identifies number of samples without COG
  if(empty > 7){
    # If COG appears in only 1 sample
    cog.rem <- append(cog.rem, i)
      # Create list of COGs that appear in < 3 samples
  }
}
cog.rem.row <- as.numeric(cog.rem)
  # Convert list to numeric

# Remove COGs that appear in < 3 samples
cog.norm.rem <- cog.norm[-cog.rem.row,]
...
cog.df <- data.frame(t(cog.norm))
rownames(cog.df) <- c("3715", "3717", "3723", "3733", "3734", "3744", "3772", "3773", "3775")
cog.df$Diet <- c("Feed", "Feed", "Fast", "Fast", "Feed", "Feed", "Fast", "Feed", "Feed")
cog.df$Diet <- ordered(cog.df$Diet, levels=c("Feed", "Fast"))
cog.mat <- as.matrix((cog.df))
DietVar <- cog.df$Diet

# Run Wilcoxon on all COGs
cog.wil <- data.frame(matrix(nrow = 1, ncol = nrow(cog.norm.rem)))
colnames(cog.wil) <- rownames(cog.norm.rem)
for(i in 1:nrow(cog.norm.rem)){
  cogName <- colnames(cog.df)[i]
  wil <- wilcox.test(as.numeric(cog.mat[,i]) ~ DietVar, cog.mat)
    cog.wil[,i] <- wil$p.value
}
cog.wil[2,] <- p.adjust(cog.wil[1,], method = "fdr")
which(cog.wil[2,] < 0.05)
